...
The core of this project is the repository-root protocol kit, especially:
...
1. Check the script syntax: `bash -n "scripts/init-kb.sh"`
2. Check the health-check script syntax: `bash -n "scripts/check-kb.sh"`
3. Initialize a test knowledge base: `bash scripts/init-kb.sh /tmp/test-kb`
   Or use the root wrapper: `bash init.sh /tmp/test-kb`
4. Initialize another test knowledge base at a path containing `&`: `bash scripts/init-kb.sh "/tmp/test&kb"`
...
7. Verify that `/tmp/test-kb/meta/schemas/` contains the canonical schema files, including `lint-report.md` when it exists in `schemas/`
8. Verify that `meta/lint-status.json` starts with `"last_lint": null` and `"lint_count": 0` for a freshly initialized knowledge base
9. Run the optional health check against both `example-kb` and `/tmp/test-kb`: `bash scripts/check-kb.sh example-kb` and `bash scripts/check-kb.sh /tmp/test-kb`
